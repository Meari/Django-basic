...
> 构造链表类(原作者所写有错误，以下代码没有问题，指针在计算长度时已经走到了尾，下面无法再查找了。)
...
## 12 找零问题
> 动态规划：逐步进行，从小问题开始递进，用小问题的答案合成大问题的答案。

我们用d(i) = j 表示凑够i元最少需要0个硬币。

d(0) = 0

d(1) = d(n-1)(拿起一个一元硬币剩余的钱数)+ 1(这个1是拿取的那枚一元硬币)
...
(4)两个指针向后移动，上一节点改为当前节点，当前节点改为下一节点。
